...
from telethon.tl.types import InputGroupCall, Channel, Chat, DataJSON
...
async def cmd_joingroupall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(
        f"⏳ *{total} accounts* join ho rahe hain...",
        parse_mode="Markdown"
    )
    results = []
    success = 0
...
async def cmd_leavegroupall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(clients)
    msg = await update.message.reply_text(
        f"⏳ *{total} accounts* leave kar rahe hain...",
        parse_mode="Markdown"
    )
    results = []
    success = 0
...
async def cmd_joinvcall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(clients)
    msg = await update.message.reply_text(
        f"⏳ *{total} accounts* VC join kar rahe hain...",
        parse_mode="Markdown"
    )

    # Pehle check karo VC active hai ya nahi
    if clients:
    ...
    for i, c in enumerate(clients, 1):
        ...
        try:
            ...
            if not call:
                results.append(f"⚠️ {me.first_name} — group mein nahi / VC nahi mili")
                continue
            ...
            if "already" in err.lower():
                results.append(f"✅ {me.first_name} — `{me.id}` (already joined)")
                success += 1
            else:
...
async def cmd_leavevcall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(clients)
    msg = await update.message.reply_text(
        f"⏳ *{total} accounts* VC chhod rahe hain...",
        parse_mode="Markdown"
    )
    results = []
    success = 0
